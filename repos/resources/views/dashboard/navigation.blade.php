<router-link exact tag="h3"
    ...
    class="cursor-pointer flex items-center font-normal dim text-blue-900 mb-8 text-base no-underline">
    ...
    <span class="text-blue-900 sidebar-label">{{ __('Dashboard') }}</span>
</router-link>
    ...
    <ul class="list-reset mb-8">
        @foreach (\Laravel\Nova\Nova::availableDashboards(request()) as $dashboard)
            <li class="leading-wide mb-4 ml-8 text-sm">
                ...
                exact
                class="text-blue-900 no-underline dim">
                    {{ $dashboard::label() }}
                </router-link>
            </li>
        @endforeach
